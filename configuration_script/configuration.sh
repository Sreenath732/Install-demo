...
select yn in "Yes" "No"; do
	...
	echo "Step 9/11 Enable retention policy to remove the data which is older than user provided days for data_table"
	if [ "$dir" != "/" ]; then
	...
	echo "Do you wish to add retension policy for data_table"
	select yn in "Yes" "No"; do
		case $yn in
	        Yes )
				...
				status=$?
				if [ $status -ne 0 ];then
				   echo "There was no retention policy on data_table to remove"	
				fi
				if [ $status -eq 0 ];then
				...
				break;;
	        No ) 
			echo "skipped retention policy for data_table"
			break;;
	    esac
	done
...
select yn in "Yes" "No"; do
	...
	echo "Do you wish to update/add retension policy for tables"
	select yn in "Yes" "No"; do
		case $yn in
			Yes )
			sudo -u postgres psql -d 'plc_data' -c "SELECT remove_retention_policy('data_table')";
			...
			if [ $alarms -eq 640 ];then
				...
				if [ $ndays -le $tocheck_data ];then
					...
					echo "Enabled retention policy to remove the data which is older than $ndays days from data_table"
			     	...
			     	echo "The suggested number of days "$tocheck_data
			     	read -p "Kindly enter the number of days of older alarms to keep in the database less than suggested number of days: " ndays
			     	sudo -u postgres psql -d 'plc_data' -c "SELECT add_retention_policy('data_table', INTERVAL '$ndays days')"; 
			     	status=$?
					if [ $status -ne 0 ];then
					   echo "Unable to add retention policy"
				...
				read -p "Kindly enter the number of days of older alarms data into alarm_table to keep in the database: " ndays
				if [ $ndays -le $tocheck_alarms_640 ];then
					...
					echo "Enabled retention policy to remove the alarms data which is older than $ndays days from alarm_table"
				else
			     	echo "The suggested number of days "$tocheck_alarms_640
			     	read -p "Kindly enter the number of days of older alarms data to keep in the database less than suggested number of days: " ndays
			     	sudo -u postgres psql -d 'plc_data' -c "SELECT add_retention_policy('alarm_table', INTERVAL '$ndays days')"; 
			     	status=$?
					if [ $status -ne 0 ];then
					   echo "Unable to add retention policy"
			...
			break;;
			No ) 
			echo "skipped retention policies for both data_table and alarm_table"
			break;;
		esac
	done
